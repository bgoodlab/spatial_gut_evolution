...
pug_qwm_axis.set_title('  Diffusion-dominated growth',fontsize=7)
ttl = pug_qwm_axis.title
ttl.set_position([.5, 0.95])

pug_spa_axis = plt.Subplot(fig, pug_grid[1])
fig.add_subplot(pug_spa_axis)
pug_spa_axis.set_xlabel('Position, $x$')
pug_spa_axis.set_title('Flow-dominated growth',fontsize=7)
ttl = pug_spa_axis.title
ttl.set_position([.5, 0.95])

param_grid = gridspec.GridSpecFromSubplotSpec(2,1,height_ratios=[1,1],hspace=0.3, subplot_spec=outer_grid[2])
...
def prefactor(zs,k):
    ...
    return (numpy.cos(k*zs)+1.0/k*numpy.sin(zs*k))

def dprefactor(zs,k):
    return (-k*numpy.sin(k*zs)+numpy.cos(zs*k))
    

#def I3(alpha):
    
#    
#    prefactors = numpy.clip(prefactor(zs,k)    
...
def neovern(k,ellstar,washout):
    I1= (  ((k**2*ellstar + ellstar+1)/(2*k**2))
     + ((k**2-1)/(4*k**3))*np.sin(2*k*ellstar) 
         - np.cos(2*k*ellstar)/(2*k**2)   )

    I2= ( (1.0/2)*(np.cos(k*ellstar)+np.sin(k*ellstar)/k)**2 )

    I3=( (np.exp(-ellstar)/(36*k**5+4*k**3))*(np.sin(k*ellstar)*(27*k**4-24*k**2-3)
        +np.sin(3*k*ellstar)*(3*k**4-12*k**2+1) 
        - np.cos(k*ellstar)*(54*k**3+6*k) 
        - np.cos(3*k*ellstar)*(10*k**3-6*k)) +(16)/(9*k**2+1))

    I4=( (2.0/washout)*((k**2+1)/k)**2 * ((np.exp(-ellstar)/(36*k**5+40*k**3+4*k))
        *(np.sin(k*ellstar)*(9*k**4+26*k**2-3)
        -np.sin(3*k*ellstar)*(3*k**4+2*k**2-1) 
        - np.cos(k*ellstar)*(36*k**3+4*k) 
        + np.cos(3*k*ellstar)*(4*k**3+4*k)) +(8*k**2)/(9*k**4+10*k**2+1)))

    I5=( (2.0/washout)*np.exp(-ellstar)*(np.cos(k*ellstar)+np.sin(k*ellstar)/k)**3)
    
    return (np.exp(-ellstar)/(np.cos(k*ellstar)+np.sin(k*ellstar)/k))*((I1*(I1+I2))/(I3+I4+I5))
...
schem_nut_axis.plot([0.22,0.28],[1,1],'k:')
schem_nut_axis.text(0.14,0.9,'$\\lambda$',fontsize=8)
...
schem_pos_axis.text(0.68,1.09,'$\\ell$',fontsize=10)
...
lame_axis.semilogx(washout_list-1, eff_lam, color='black')
...
# small window
lame_axis.set_xlim([2e-02,5e01])
lame_axis.set_ylim([5e-02,2])


lame_axis.set_xticklabels([])
Nes=[]
#washout_list=np.linspace(1.04,1000,100000)
washout_list = 1+np.logspace(log10(0.001),2,100) 
#washout_list = 1+np.array([1e-04,1e-03,1e-02,1e-01])
for washout in washout_list:
    k=np.sqrt(washout-1)
    gamma=calculate_gammas(k)
    ellstar=gamma/k
    
    
    Nes+=[neovern(k,ellstar,washout)]
...
ne_axis.loglog(washout_list-1, Nes, 'k-')
ne_axis.tick_params()

#ne_axis.semilogx(x3_asym*1.5,(np.pi**2/10)*np.power(x3_asym,-3)*np.exp(-np.pi/(np.sqrt(x3_asym))), 'k:')
#ne_axis.semilogx(x2_asym, 0.1*np.ones(100),'k:')

xs = np.logspace(-2,2,100)
ne_axis.loglog(xs,np.ones_like(xs)*0.25,'r:')
#xs = np.logspace(-2,np.log10(np.square(np.pi/6))+0.3, 100)
xs = np.logspace(-2,-0.25, 100)
ne_axis.loglog(xs,np.pi**2/(2**7)*np.exp(2)*(np.power(xs,-3))*np.exp(-np.pi/np.sqrt(xs)),'r:')

# big window
#ne_axis.set_xlim([1e-02,1e02])
#ne_axis.set_ylim([1e-08,1e02])

# small window
ne_axis.set_xlim([2e-02,5e01])
ne_axis.set_ylim([1e-04,1e00])

schem_nut_axis.text(-0.1, 1.1, 'a', horizontalalignment='center', verticalalignment='center', transform=schem_nut_axis.transAxes,fontweight='bold',fontsize=7)


schem_pos_axis.text(-0.1, 1.1, 'b', horizontalalignment='center', verticalalignment='center', transform=schem_pos_axis.transAxes,fontweight='bold',fontsize=7)

pug_qwm_axis.text(-0.005, 1.11, 'c', horizontalalignment='center', verticalalignment='center', transform=pug_qwm_axis.transAxes,fontweight='bold',fontsize=7)


pug_spa_axis.text(-0.005, 1.11, 'd', horizontalalignment='center', verticalalignment='center', transform=pug_spa_axis.transAxes,fontweight='bold',fontsize=7)


lame_axis.text(-0.5, 1.05, 'e', horizontalalignment='center', verticalalignment='center', transform=lame_axis.transAxes,fontweight='bold',fontsize=7)

ne_axis.text(-0.5, 1.05, 'f', horizontalalignment='center', verticalalignment='center', transform=ne_axis.transAxes,fontweight='bold',fontsize=7)
