...
establishment_grid = gridspec.GridSpecFromSubplotSpec(3,3, subplot_spec=outer_grid[0],height_ratios=[0.55,1.2,0.45],width_ratios=[0,1,0.6])

establishment_axis = plt.Subplot(fig, establishment_grid[1,1])
fig.add_subplot(establishment_axis)
...
establishment_axis.plot(extinct_ts,extinct_logns,'r-',alpha=0.5)
establishment_axis.plot(established_ts,established_logns,'r-')
establishment_axis.plot([0.65,0.9],[0.62,0.62+s*0.25],'k:')
establishment_axis.set_xlim([established_ts.max()*(-0.1),established_ts.max()])
establishment_axis.set_ylim([0,established_logns.max()])
establishment_axis.set_xticks([])
establishment_axis.set_yticks([])
establishment_axis.set_xlabel('Time, $t$')
establishment_axis.set_ylabel('Log $f(t)$')

#establishment_axis.text(0.5,0.3,'$p_\\mathrm{fix} \\propto \\int \\lambda(x) w_s(x) \\rho(x)$')

establishment_axis.text(0.6,0.3,'$p_\\mathrm{fix}$')


#pylab.text(0.85,0.62,'$s_e$')
establishment_axis.text(0.92,0.78,'$s_e$')

establishment_axis.spines['top'].set_visible(False)
establishment_axis.spines['right'].set_visible(False)

establishment_axis.text(-0.11, 1.05, 'a', horizontalalignment='center', verticalalignment='center', transform=establishment_axis.transAxes,fontweight='bold',fontsize=7)
